...
<template>
  <div>
    <v-data-table :loading="loading" :headers="computedHeaders" :items="computedItems"
      :server-items-length="totalItems" :options.sync="options" :search="search" multi-sort
      :footer-props="{ 'items-per-page-options': [ 10, 25, 50, 100 ]}">
      <!-- Top -->
      <template v-slot:top>
        <v-row dense class="px-2 pt-2">
          <v-col cols="12">
            <v-text-field hide-details dense clearable append-icon="mdi-magnify" label="Search" outlined v-model="search" @input="updateSearch" />
          </v-col>
        </v-row>
      </template>
      <!-- Actions -->
    ...
    <ModalConfirm v-model="deleteDialog" :title="`Delete ${name}`" :message="itemToDelete ? `Delete ${name} <strong>${itemToDelete.id}</strong>?` : `Delete ${name}`"
  ...
  async fetch() {
    ...
    if (response.headers) {
      if (response.headers.hasOwnProperty('content-range'))
        this.totalItems = this.parseContentRange(response.headers['content-range']).length
      else
        this.totalItems = 0
    } else
      this.totalItems = 0
  ...
  computed: {
    ...
    computedHeaders() {
      ...
      if (this.headers && this.headers.length)
        ...
        return [...this.headersFromItems(), actionHeader]
    },
  },
  methods: {
    async updateSearch(val) {
      this.debounce(() => { this.$fetch(); this.options.page = 1 }, 200)
    },
    confirmDelete(item) {
    ...
    async deleteItem() {
      ...
      try {
        let response = await this.$axios.$delete(this.endpoint + '/' + this.itemToDelete.id)
        this.$fetch()
      } catch (err) {
      }
    },
    ...
    createParams() {
      ...
      if (this.options) {
        if (this.options.page == 1)
          ...
          params.offset = ((this.options.page-1) * this.options.itemsPerPage) || 0
      ...
      if (this.search)
        params.q = encodeURI(this.search)
